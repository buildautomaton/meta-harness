 ...
 * Override via RUNTIME_PLANS_DIR if the host wants a custom root.
 */
export function getSessionPlansRootDir(): string {
  const override = process.env.RUNTIME_PLANS_DIR?.trim();
  if (override) return path.resolve(override);
  return path.join(os.tmpdir(), 'runtime-plans');
}
