...
export const AWAIT_MINION_DEFINITION: McpToolDefinition = {
  ...
  description:
    'Block until a minion finishes or needs the user, streaming progress as MCP notifications on this tool call. Use after spawn_minion with background=true, or after resolve_minion_request. Do not poll get_minion in a loop — await instead. Returns compacted agent messages only.',
  inputSchema: {
    type: 'object',
    properties: {
