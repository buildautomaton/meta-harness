...
export function listDecisions(db: SqlStore, workId: string): string[] {
  return all(db, 'SELECT text FROM work_decision WHERE work_id = ? ORDER BY ordinal ASC', [workId]).map((row) =>
    String(row.text),
  );
}

export function replaceDecisions(db: SqlStore, workId: string, decisions: string[]): void {
...
export function appendDecision(db: SqlStore, workId: string, text: string): void {
...
export function listWorkQuestions(db: SqlStore, workId: string): DesignQuestion[] {
